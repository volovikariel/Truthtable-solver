/**
 * Dear Reader, 
 *     After looking at this code, a couple months after I had written it quickly over a week.
 *     I realize that I should instead have simplified the algorithm a lot more, using Stacks rather than checking for all possible cases.
 *     However, this was an opportunity to develop my own way of solving this and so that is what I had done.
 *     I have little need of this program now, so I will not rewrite it for now, it has served me well.
 *
 *     To any person looking at this code to see whether I'm a good candidate for a position, this code may not be the best representation of my abilities.
 * - December 23 2020
**/
let inputStr = '((~p v q) ^ ((~p v q) -> r))'; 

let predicates = "";
for(let i = 0; i < inputStr.length; i++) {
    // Check if the character is a letter and isn't v or V
    if((inputStr[i].charCodeAt(0) > 64 && inputStr[i].charCodeAt(0) < 91) || (inputStr[i].charCodeAt(0) > 96 && inputStr[i].charCodeAt(0) < 123) && !inputStr[i].includes('v') && !inputStr[i].includes('V')){
        if(!predicates.includes(inputStr[i])){
...
function _recursFunc(expression) {
    ...
    twoParentheses = (twoParentheses !== null) ? twoParentheses.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) : twoParentheses;
    oneParenthesisLeft = (oneParenthesisLeft !== null) ? oneParenthesisLeft.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) :  oneParenthesisLeft;
    oneParenthesisRight = (oneParenthesisRight !== null) ? oneParenthesisRight.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) : oneParenthesisRight;
    noParenthesis = (noParenthesis !== null) ? noParenthesis.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) : noParenthesis;
    rightNegatedExpress = (rightNegatedExpress !== null) ? rightNegatedExpress.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) : rightNegatedExpress;
    leftNegatedExpress = (leftNegatedExpress !== null) ? leftNegatedExpress.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) :  leftNegatedExpress;
    bothNegatedExpress = (bothNegatedExpress !== null) ? bothNegatedExpress.map((curr) => (typeof curr == 'string') ? curr.trim() : curr) : bothNegatedExpress;
    let left;
    let operator;
    let right;
    ...
    if(twoParentheses != null) {
        ...
        if(expression.length == 1) {
            ...
            if(process.dev) {
                console.log('Unknown expression format');
            }
        }
    }
}
